...
// tslint:disable-next-line: no-implicit-dependencies : disable-next-line: max-line-length
import { createIdentifier, Identifier, LiteralLikeNode, MethodSignature, Node, PropertySignature, SyntaxKind, TypeReferenceNode } from 'typescript';
...
export class TestGenerator {
  ...
  generateTests(context: GeneratorContext): GeneratorResult {
    ...
    // Header
    const header = NodeGenerator.getMultiLineComment(
  ...
  generateMethodCall(classDescriptor: ClassDescriptor, method: MethodSignature): MemberGeneratorResult {
    ...
    const methodReturnsResult = this.isMethodWithResult(method);
    const methodIsAsync = this.isAsyncMethod(method);
    ...
    if (methodIsAsync) {
      result.memberType = methodReturnsResult ? MemberType.AsyncFunction : MemberType.AsyncMethod;
    } else {
    ...
    const callExpression = NodeGenerator.getMethodCall(
      methodName,
      instance.memberName,
      methodIsAsync,
      argumentNames,
    ...
    result.statements.push(callExpression);
  ...
  isMethodWithResult(method: MethodSignature): boolean {
    ...
    switch (method.type.kind) {
      ...
      case SyntaxKind.TypeReference:
        const isPromise = (<TypeReferenceNode>method.type).typeName.getText() === 'Promise';

        if (isPromise) {
          // What's the return type of promise?
          const typeArgument = (<TypeReferenceNode>method.type).typeArguments[0];

          // If return type is not void, we've a return value.
          return typeArgument.kind !== SyntaxKind.VoidKeyword;
        } else {
          // Type is given and not a promise. It's returning someting.
          return true;
        }
  ...
  generatePropertyCall(classDescriptor: ClassDescriptor, property: PropertySignature): MemberGeneratorResult {
    ...
    // Get type descriptor
    const variableDescriptor = NodeAnalyser.getVariableDescriptor(property, (name) => this.getName(tools.camelCase(name)));
